...
routes: ROUTES_KEYWORD var_name curly_block ;
routes_through: ROUTES_KEYWORD var_name (THROUGH_KEYWORD (var_name ((COMMA var_name)+)?))? curly_block ;
...
constructor : CONSTRUCTOR_KEYWORD var_name var_name_with_params (AS_KEYWORD var_name)? ;
...
ARRAY_TYPE_SUFFIX : [[][\]] ;
param_values : EMPTY_PARENTHESIS | vars_with_params_special ;
...
update : update_declaration function_curly_block ;
UPDATE_TYPE : ('update') ;
update_declaration : (UPDATE_TYPE) (name_declaration var_args) ;
DOT_SEPARATOR: [.] ;
function_call : (any_var (((DOT_SEPARATOR) any_var)+)?) param_values ;
VOID_TYPE : ('void') ;
function_types: VOID_TYPE | var_type ;
...
return: RETURN_KEYWORD (WS return_value?) ;
isset : NOT? (ISSET_KEYWORD | empty_function);
ISSET_KEYWORD : ('isset') | ('empty') ;
empty_function : NOT? ISSET_KEYWORD ( OPEN_PARENTHESES any_var CLOSE_PARENTHESES) ;
...
VAR_TYPES : ('string') | ('number') | ('bool') | ('boolean') | ('dynamic') | ('function') | ('time') ;//PROPERTY_VALUES : (number | QUOTED_STRING | path | HTML_ELEMENT | NAMED | NAMED_DOT | (NAMED_VAR | NAMED) | NULL_VALUE | '*') ; // | BRACES_ARRAY_BLOCK | NAMED_PROPERTY_OBJECT | NAMED_PROPERTY ;
property_value : component_tree | var_name_with_params | path | array  | expression | (number | string | html_element | any_var | static_var | NULL_VALUE | QUERY_SELECT_ALL ) ;
QUERY_SELECT_ALL : '*' ;
static_var : (var_name ((STATIC_ACCESS var_name)+)) ;
return_property_value : var_name_with_params | array | expression | path | number | string | html_element | NAMED | NAMED_DOT | (NAMED_VAR | NAMED) | NULL_VALUE ;
...
vars_with_params_special : (OPEN_PARENTHESES) ((var_param COMMA?)+) ((COMMA VARIABLE_ARGS var_param))? CLOSE_PARENTHESES ;
var_param : string | number | array | expression | html_element | var_name ;
VARIABLE_ARGS : '...' ;
