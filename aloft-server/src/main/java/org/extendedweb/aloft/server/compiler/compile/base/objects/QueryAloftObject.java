...
public class QueryAloftObject extends AloftObject {

    private DbObject connection;

    public QueryAloftObject(ParserRuleContext ctx, CompiledObjectsRegister register, File file) throws CompilerException {
        super(ctx, register, file);
    }

    public static QueryAloftObject createIf(List<AloftParser.SyntaxContext> root, CompiledObjectsRegister register, int index, File file) {
        try {
            ...
            return new QueryAloftObject((ParserRuleContext) getContextClass().cast(ctx), register, file);
